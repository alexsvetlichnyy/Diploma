...
int main()
{
  ...
  fParam->SetMinExPerNucleounForMF(5*MeV);
...
if((histoManager.GetUpB() > 0 && histoManager.GetLowB() >= 0) && (histoManager.GetUpB() > histoManager.GetLowB()) && histoManager.GetUpB() < rA_plus_rB+7.5) {
  ...
  G4double MorisseyDev0 = 2*73*MeV; //Model parameter
  //VanBiber model parameter
  G4double VBDev1 = 146*MeV; //Model parameter

for(G4int count=0;count<histoManager.GetIterations() ;count++){ 
  id = count;
  //An event generated by GlauberMC is here.
  //bGen = std::sqrt((histoManager.GetUpB() * histoManager.GetUpB() - histoManager.GetLowB() * histoManager.GetLowB()) * randFlat.fire() + histoManager.GetLowB()*histoManager.GetLowB());
  mcg->Run(1);
  ...
  if(!(A == 0 && Ab ==0)) {
   ...
   if(0) {
       ...
       p = randGauss.shoot() * GoldhaberDev;
   }

   //Morissey model
    if(1){
        G4double MorisseyDev = MorisseyDev0 * pow(NpartA,0.25);
        p = randGauss.shoot()*MorisseyDev*A;
    }

    //VanBiber+Morissey model. Due to K. Van Bibber et al. PRL v.43, 840 (1979) VB term corresponds fragmentation of a projectile
      if(0){
          G4double MorisseyDev = MorisseyDev0 * pow(NpartA,0.25);
          G4double VBDev = VBDev1*pow(A*(A-1)/(sourceA*(sourceA-1)),0.5);
          p = randGauss.shoot()*std::sqrt(MorisseyDev*MorisseyDev + VBDev*VBDev);
      }
